...
import Circle from 'pixi/views/Circle.js';
...
function pixiPianoRoll(opt, app) {
  var colors = {
    blackKey: 0x000000,
    whiteKey: 0xffffff,
  ...
  function getNoteRange() {
    ...
    try {
      for (
        var _iterator = opt.noteData[Symbol.iterator](), _step;
  ...
  function drawPianoKey(key, active) {
    return (key.graphic || new pixi.Graphics())
      ...
      .lineStyle(
        key.color === colors.whiteKey ? noteHeight / 10 : 0,
        colors.blackKey
      )
    ...
    for (var i = noteRange.min; i < noteRange.max + 2; i++) {
      var y = opt.height + (noteRange.min - i) * noteHeight,
        ...
        key = {
          keyNumber: note.key(),
          activeColor:
            typeof opt.noteColor === "number"
              ? opt.noteColor
              : opt.noteColor[chroma],
        };
      ...
      if (new Set([0, 2, 4, 5, 7, 9, 11]).has(chroma)) {
        whiteKeys.push(
          Object.assign(key, {
            ...
            height: new Set([2, 7, 9]).has(chroma)
              ? noteHeight * 2
              : noteHeight * 1.5,
  ...
  function drawNotes() {
    ...
    try {
      for (
        var _iterator2 = opt.noteData[Symbol.iterator](), _step2;
        ...
        var pixiNote = new pixi.Graphics(),
          ...
          color =
            typeof opt.noteColor === "number"
              ? opt.noteColor
              : opt.noteColor[teoriaNote.chroma()];
        pixiNote
          .beginFill(color)
        ...
        pixiNote.y =
          opt.height -
          (keyNumber - noteRange.min) * noteHeight +
          halfGridLineWidth;
    ...
    noteContainer.x = oldContainer
      ? oldContainer.x
      : transportTimeToX(opt.time);
  ...
  function moveVerticalGridLines(horizontalMovement) {
    ...
    try {
      for (
        var _iterator3 = verticalGridlines[Symbol.iterator](), _step3;
    ...
    if (verticalGridlines[0].x + gridLineWidth < opt.pianoKeyWidth) {
      ...
      line.x =
        verticalGridlines[verticalGridlines.length - 1].x + gridLineSpacing;
  ...
  function drawGridlines(type) {
    ...
    if (!type || type === "vertical") {
      var offset = getFirstVerticalGridLineX(
  ...
  function activateKeys() {
    ...
    try {
      for (
        var _iterator4 = noteContainer.children[Symbol.iterator](), _step4;
    ...
    try {
      for (
        var _iterator5 = intersectedKeys[Symbol.iterator](), _step5;
    ...
    try {
      for (
        var _iterator6 = activeKeys[Symbol.iterator](), _step6;
